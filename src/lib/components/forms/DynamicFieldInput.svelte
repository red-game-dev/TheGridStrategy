<script lang="ts">
  ...
  export let strategyKey: string = 'grid';
  ...
  async function handleInput(event: Event) {
    ...
    if (strategy && value.trim()) {
      try {
        const fieldErrors = DynamicSchemaBuilder.validateField(strategy, fieldDefinition.binding, value);
  ...
  function getInputTypeFallback(binding: string): string {
    if (binding.includes('ratio') || binding.includes('growth') || binding.includes('size') || binding.includes('seconds')) {
      return 'number';
    }
    return 'text';
  }
...
<div class="dynamic-field-input" data-testid="dynamic-field-input">
  <label for={`field-${fieldDefinition.binding}`} class="block text-sm font-medium text-gray-700 mb-2" data-testid="field-label">
    ...
    {#if fieldDefinition.description}
      <span class="block text-xs text-gray-500 font-normal mt-1" data-testid="field-description">
    ...
    {#if fieldDefinition.presets && fieldDefinition.presets.length > 0}
      <div class="flex items-center gap-2 text-sm" data-testid="field-presets">
        ...
        <select
          ...
          class="text-sm border border-gray-300 rounded px-2 py-1 bg-white hover:bg-gray-50 focus:ring-1 focus:ring-blue-500 focus:border-blue-500"
