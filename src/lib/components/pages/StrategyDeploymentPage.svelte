<script lang="ts">
    ...
    async function initializeApp() {
        try {
            ...
            initializationError = error instanceof Error ? error.message : 'Failed to initialize application';
        } finally {
     ...
     */
     function handleTokenIOChange(event: CustomEvent<{ index: number; label: string; value: string }>) {
     ...
     */
     async function handleFormSubmit() {
      ...
      try {
        ...
        const transactionHash = await sendDeploymentTransaction(orderbookAddress, deploymentCalldata as Hex);
  ...
  {:else}
    <main class="space-y-8 py-8">
      {#if shouldShowDebug('debugPanels')}
        ...
        <div slot="content" class="grid grid-cols-2 gap-4 mt-2">
          <div>
          ...
          <p class="text-red-700 text-sm">{gui.error}</p>
        </div>
        
        <div slot="controls" class="flex gap-2 mt-2">
          <button 
            on:click={resetGuiState}
            class="px-3 py-1 bg-yellow-200 text-yellow-800 rounded text-xs hover:bg-yellow-300"
          >
            Reset GUI
          </button>
          <button 
            on:click={() => guiStore.setError(null)}
            class="px-3 py-1 bg-blue-200 text-blue-800 rounded text-xs hover:bg-blue-300"
          >
            Clear Errors
          </button>
          <button 
            on:click={() => debugLog.log('Current state:', { wallet: $walletStore, strategy: $strategyStore, gui: $guiStore })}
            class="px-3 py-1 bg-green-200 text-green-800 rounded text-xs hover:bg-green-300"
          >
            Log State
          </button>
        </div>
      ...
      <section class="bg-white rounded-lg shadow-md p-6">
        <h2 class="text-xl font-semibold text-gray-800 mb-4">Select Network</h2>
        <NetworkSelector 
      ...
      {#if strategy.allTokensSelected}
        ...
        <section class="bg-white rounded-lg shadow-md p-6">
            <label class="flex items-center space-x-3">
              ...
              />
              <span class="text-gray-700 font-medium">Show advanced options</span>
            </label>
          </section>
          ...
          {#if strategy.showAdvancedOptions}
            {#if gui.fieldDefinitionsWithDefaults.length > 0}
              <section class="bg-white rounded-lg shadow-md p-6">
                <h2 class="text-xl font-semibold text-gray-800 mb-4">Advanced Parameters</h2>
                <div class="space-y-6">
            ...
            {#if gui.tokenInputs.length > 0 || gui.tokenOutputs.length > 0}
              <section class="bg-white rounded-lg shadow-md p-6">
                  <h2 class="text-xl font-semibold text-gray-800 mb-4">Token I/O Configuration</h2>
                  <p class="text-sm text-gray-600 mb-4">
                      Configure vault IDs for your token inputs and outputs. This is optional for advanced users who want to specify custom vault behavior.
                  </p>
                  ...
                  <div class="space-y-6">
                      ...
                      <div>
                          ...
                          <div class="space-y-4 pl-4 border-l-2 border-red-100">
                      ...
                      <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                          <h4 class="text-sm font-semibold text-blue-800 mb-2">What are Vault IDs?</h4>
                          <p class="text-sm text-blue-700 mb-2">
                              Vault IDs allow you to specify custom vault behavior for your token inputs and outputs. 
                              This is an advanced feature that most users can leave empty.
                          </p>
                          <ul class="text-xs text-blue-600 space-y-1">
                              <li>• <strong>Output Vaults:</strong> Where tokens you're selling are stored</li>
                              <li>• <strong>Input Vaults:</strong> Where tokens you're buying will be deposited</li>
  ...
  {#if showRainlangModal}
    <RainlangModal
      code={rainlangCode}
      on:close={() => showRainlangModal = false}
    />
  {/if}